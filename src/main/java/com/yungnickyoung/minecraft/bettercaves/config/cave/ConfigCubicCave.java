...
public class ConfigCubicCave {
    ...
    @Config.Comment("Determines how frequently Type 1 Caves spawn. If this is anything but VeryCommon (the default), vanilla caves will " +
            "replace some of the areas where Type 1 Caves would have generated. However, if vanilla caves are disabled, " +
            "then these areas will instead not have any caves at all.")
    @Config.RequiresWorldRestart
    public CaveFrequency caveFrequency = CaveFrequency.VeryCommon;

    @Config.Name("Type 1 Cave Frequency Custom Value")
    @Config.Comment("Custom value for cave frequency. Only works if Type 1 Cave Frequency is set to Custom. 0 = 0% chance of spawning, " +
