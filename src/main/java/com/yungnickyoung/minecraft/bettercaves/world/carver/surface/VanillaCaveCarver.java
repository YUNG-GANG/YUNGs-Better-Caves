...
public class VanillaCaveCarver extends MapGenCaves implements ICarver {
    ...
    public void generate(World worldIn, int x, int z, ChunkPrimer primer, boolean addRooms) {
        int i = this.range;
        this.world = worldIn;
        ...
        for (int l = x - i; l <= x + i; ++l)
        {
            for (int i1 = z - i; i1 <= z + i; ++i1)
            {
                ...
                this.recursiveGenerate(worldIn, l, i1, x, z, primer, addRooms);
            }
        }
    }

    protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, @Nonnull ChunkPrimer chunkPrimerIn, boolean addRooms) {
    ...
    public void generate(World worldIn, int x, int z, @Nonnull ChunkPrimer primer) {
        generate(worldIn, x, z, primer, false);
    }

    @Override
    protected void recursiveGenerate(World worldIn, int chunkX, int chunkZ, int originalX, int originalZ, ChunkPrimer chunkPrimerIn) {
        recursiveGenerate(worldIn, chunkX, chunkZ, originalX, originalZ, chunkPrimerIn, false);
    }
