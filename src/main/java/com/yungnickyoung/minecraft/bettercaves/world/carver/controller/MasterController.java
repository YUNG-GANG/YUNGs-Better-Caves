...
public class MasterController {
    ...
    public boolean carveRegion(Chunk chunkIn, Function<BlockPos, Biome> biomePos, int chunkX, int chunkZ, BitSet airCarvingMask, BitSet liquidCarvingMask) {
        ...
        for (int x = 0; x < 16; x++) {
            for (int z = 0; z < 16; z++) {
                surfaceAltitudes[x][z] = configHolder.overrideSurfaceDetection.get()
                    ...
                    : Math.min(
                    chunkIn.sampleHeightmap(Heightmap.Type.WORLD_SURFACE_WG, x, z),
                    chunkIn.sampleHeightmap(Heightmap.Type.OCEAN_FLOOR_WG, x, z));
            }
        }
